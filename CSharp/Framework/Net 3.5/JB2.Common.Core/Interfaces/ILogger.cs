...
namespace JB2.Common
{
    ...
    public interface ILoggerAsync<TServerity, TKey, TLogEntry> : ILogger<TServerity, TKey, TLogEntry>
        ...
        where TLogEntry : ILogEntry<TKey, TServerity>
    {
        void LogAsync(TLogEntry entry);

        void LogDebugMessageAsync(string message, string logcode = "");

        void LogErrorAsync(Exception ex, string message = "", string logcode = "");

        void LogMessageAsync(string message, string logcode = "");
    }

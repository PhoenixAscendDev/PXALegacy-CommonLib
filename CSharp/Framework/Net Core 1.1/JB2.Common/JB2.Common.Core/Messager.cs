...
namespace JB2.Common
{
        ...
        where TMessageKey : IComparable
    {
        ...
        {
            return DeliverAsync(message).Result;
        }

        public virtual async Task<ServiceResult<TMessage>> DeliverAsync(TMessage message)
        {
            ...
            try
            {
                var d = await Task.Run(() => SendMessageDelegate()?.Invoke(message));
                if(d)

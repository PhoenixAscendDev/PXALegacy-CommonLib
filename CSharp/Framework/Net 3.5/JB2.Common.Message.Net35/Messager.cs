...
namespace JB2.Common.Message
{
        ...
        where TMessage : JB2.Common.Message.IMessage
    {
        ...
        {
            return RecieveMessagesAsync(mailbox);
        }


        public ServiceResult<IEnumerable<TMessage>> RecieveMessagesAsync(TMailbox mailbox)
        {
            ...
            try
            {
                var d =  RecieveNewMessagesDelegate()?.Invoke(mailbox);
                if (d)                   
                result = d;

...
namespace JB2.Common.Log
{
    public abstract class Logger : ILoggerAsync
    {
        ...
        public virtual void LogAsync(ILogEntry entry)
        {
            if (IsEnabled(entry.Serverity))
            {
                var workResult = GetLogMethod()?.Invoke(entry);
        ...
        public abstract void LogDebugMessageAsync(string message, string logcode = "");

        public abstract void LogErrorAsync(Exception ex, string message = "", string logcode = "");

        public abstract void LogMessageAsync(string message, string logcode = "");
